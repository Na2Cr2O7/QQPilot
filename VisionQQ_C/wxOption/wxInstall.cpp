...
public:
	...
	Mainwin(wxWindow* parent = nullptr) :MyFrame1(parent)
	{
		...
		//char* w= getFile();
		//w;
		////m_system->SetValue("getFile()");
		//std::string ww(w);
		//ww;
		//m_system->SetValue(ww);
		//buffer.reset();
...
static std::string GetSystem()
{
	...
	if (fileHandle == INVALID_HANDLE_VALUE) {
		...
		size_t pos = fileContent.find("system");
		size_t previousPos = 0;
		do
		{
			previousPos = pos;
			fileContent = fileContent.substr(previousPos + 1);
			pos = fileContent.find("=");

		} while (pos != std::string::npos);

		std::cout << fileContent.substr(previousPos + 1);
		wxMessageBox(fileContent.substr(previousPos + 1));
		// 关闭文件
		CloseHandle(fileHandle);
	return fileContent.substr(previousPos + 1);
	}
	return "";
