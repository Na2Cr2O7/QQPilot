...
自 **VisionQQ v1.5 起，不再依赖 OCR 进行文字识别**，转而使用更高效的内容提取方式。

✅ 升级后可安全卸载 `easyocr`：

```bash
PythonPath.cmd -m pip uninstall easyocr
```
...
VisionQQ 是一个全自动的 QQ 聊天机器人，通过以下流程实现智能回复：
...
VisionQQ在最低配置上成功运行，使用内置语言模型进行回复
...
### 步骤 1：下载项目
前往 [Releases 页面](https://github.com/Na2Cr2O7/Visionqq/releases) 下载最新压缩包并解压。
